...
namespace Lib.Common.DataTypes
{
	...
	public class Bit : IFormattable
	{
		...
		private Bit(int bit)
		{
			if (bit<0||bit>1) throw new ArgumentInvalidException(nameof(bit), bit, Error.InvBitInt);
		...
		private Bit(string bit)
		{
			...
			if (!bit.ToLower().Equals("true")&&!bit.ToLower().Equals("false")) throw new ArgumentInvalidException(nameof(bit), bit, Error.InvBitStr);

			if (bit.ToLower().Equals("true")) value=1;
			else value=0;

		}

		/// <remarks />
		private Bit(bool bit) => value=Convert.ToInt32(bit);
